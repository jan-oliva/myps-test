...
class Eciovni extends Control {
     ...
     * @param Data $data
     */
    public function __construct(\OndrejBrejla\Eciovni\Data $data = NULL) {
        if ($data !== NULL) {
            $this->setData($data);
        }
     ...
     * @throws IllegalStateException If data has already been set.
     */
    private function setData(\OndrejBrejla\Eciovni\Data $data) {
        if ($this->data == NULL) {
            $this->data = $data;
        } else {
    ...
    private function generateSupplier(IFileTemplate $template) {
		...
		$template->supplierRegistrationText = $supplier->getRegistrationText();
    }
