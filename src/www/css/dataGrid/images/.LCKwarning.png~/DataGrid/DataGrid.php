...
class DataGrid extends Nette\Application\UI\Control implements \ArrayAccess
{
	...
	/** @var PM\DataGrid\Renderers\IRenderer */
	protected $renderer;

	/** @var PM\DataGrid\DataSources\IDataSource */
	...
	/** @var PM\DataGrid\Columns\ActionColumn */
	 ...
	 */
	public function getColumns($type = 'PM\DataGrid\Columns\IColumn')
	{
	 ...
	 * @return \ArrayIterator
	 */
	public function getFilters($type = 'PM\DataGrid\Filters\IColumnFilter')
	{
	 ...
	 * @return \ArrayIterator
	 */
	public function getActions($type = 'PM\DataGrid\IAction')
	{
		$actions = new \ArrayObject();
		foreach ($this->getColumns('PM\DataGrid\Columns\ActionColumn') as $column) {
			if ($column->hasAction()) {
	 ...
	 * @return void
	 */
	public function formSubmitHandler(Form $form)
	{
	...
	public function render()
	{
		if (!$this->wasRendered) {
			...
			if (!$this->hasColumns() || (count($this->getColumns('PM\DataGrid\Columns\ActionColumn')) == count($this->getColumns()))) {
	...
	{
		if (!$this->hasColumns('PM\DataGrid\Columns\ActionColumn')) {
			throw new InvalidStateException('No PM\DataGrid\Columns\ActionColumn defined. Use PM\DataGrid\DataGrid::addActionColumn before you add actions.');
		}
