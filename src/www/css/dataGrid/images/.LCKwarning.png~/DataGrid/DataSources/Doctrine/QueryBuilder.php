...
namespace PM\DataGrid\DataSources\Doctrine;

use Doctrine,
	Doctrine\ORM\Query\Expr,
	\PM\DataGrid\DataSources,
	\PM\DataGrid\DataSources\Utils\WildcardHelper;
...
class QueryBuilder extends DataSources\Mapped
{
	...
	public function filter($column, $operation = self::EQUAL, $value = NULL, $chainType = NULL)
	{
		...
		if(isset($this->aliasReverseMapping[$column]))
		{
			$columnMaped = $this->aliasReverseMapping[$column];
		}
		else
		{
			$columnMaped = $this->mapping[$column];
		}
		...
		if (is_array($operation)) {
			...
			foreach ($operation as $t) {
				...
				if ($t === self::IS_NULL || $t === self::IS_NOT_NULL) {
					...
					$conds[] = "{$columnMaped} $t ?$nextParamId";
			...
			if ($operation === self::IS_NULL || $operation === self::IS_NOT_NULL) {
				$this->qb->andWhere("{$columnMaped} $operation");
			} else {
				$this->qb->andWhere("{$columnMaped} $operation ?$nextParamId");
				$this->qb->setParameter(
					$nextParamId,
